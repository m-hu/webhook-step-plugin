...
public class WithWebhookStepExecution extends AbstractStepExecutionImpl {
	...
	public WithWebhookStepExecution(WithWebhookStep step, 
			StepContext context) {
        super(context);
        this.step = step;
        this.response = null;
	...
	public boolean start() throws Exception {
		//generate hook url
        token = (step == null || StringUtils.isEmpty(step.getToken()))
                ? java.util.UUID.randomUUID().toString()
                : URLEncoder.encode(step.getToken(), "UTF-8");
        listener().getLogger().println("webhook token="+ token + " generated.");
        ...
        this.setBody(bodyInvoker.
        withContext(EnvironmentExpander.merge(context.get(EnvironmentExpander.class), 
        		EnvironmentExpander.constant(overridesM))).
        withCallback(new Callback()).
        start());
        listener().getLogger().println("body script started.");
        setupHookProcessor();
		return false;
	}
    ...
    public void onTriggered(WebhookResponse response) {
        listener().getLogger().println("web hook triggered");
    ...
    private class Callback extends BodyExecutionCallback.TailCall {
        ...
        @Override protected void finished(StepContext context) throws Exception {
            if (killer!=null) {
                killer.cancel(true);
                killer = null;
                listener().getLogger().println("nested steps finished");
            }
        }
    ...
    private void cancel() {
        listener().getLogger().println("Cancelling nested steps due to webhook triggered");
        body.cancel(new WebhookTriggeredException());
    }
    
    private void setupHookProcessor() {
        listener().getLogger().println("setup hook processor");
        if (this.response == null) {
            listener().getLogger().println("register current execution");
        	this.response = WithWebhookRootAction.registerWebhook(this);
        }
    	if(killer!=null) {
    		if(this.getResponse()!=null) {
        		listener().getLogger().println("web hook response recieved");
        		listener().getLogger().println("tell killer to kill the nested process");
    			step.notify();
    		}
    		return;
    	}
    	else {
    		listener().getLogger().println("killer created");
	    	setKiller(new FutureTask<>(new Runnable() {
	            @Override
	            public void run() {
	            	while (getResponse()==null) {
	            		try {
	            			listener().getLogger().println("waiting for response");
	            			synchronized(step) {
	            				step.wait();
	            			}	 
	            			listener().getLogger().println("triggered");
	            	...
	            	if(getResponse()!=null) {
	            		listener().getLogger().println("response recieved, cancel nested process");
	            		cancel();
	            	}
	            }
	        }, getResponse()));
